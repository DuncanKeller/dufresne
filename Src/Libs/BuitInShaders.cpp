...
const char* dfPrimitiveRectangleFrag = ""
	"#version 130\n"
	"#extension GL_ARB_explicit_attrib_location : require\n"
	"#extension GL_ARB_explicit_uniform_location : require\n"
	"in vec2 texture_coordinates;"
	...
	"void main()"
	"{"
	"	frag_color = vec4(inColor.r, inColor.g, inColor.b, inColor.a);"
	"}"
	"";

const char* dfPrimitiveCircleVert = ""
	"#version 130\n"
	"#extension GL_ARB_explicit_attrib_location : require\n"
	"#extension GL_ARB_explicit_uniform_location : require\n"
	"layout(location = 0) in vec3 vertex_position;"
	...
	"	if(vertex_position.x < 0.0 && vertex_position.y < 0.0)"
	"	{"
	"		gl_Position.x = (((pos.x - radius) / resolution.x) * 2) - 1; "
	"		gl_Position.y = (((pos.y - radius) / resolution.y) * 2) - 1; "
	"		texture_coordinates.x = 0; texture_coordinates.y = 0;"
	"	}"
	"	else if(vertex_position.x > 0.0 && vertex_position.y < 0.0)"
	"	{"
...
const char* dfPrimitiveCircleFrag = ""
	"#version 130\n"
	"#extension GL_ARB_explicit_attrib_location : require\n"
	"#extension GL_ARB_explicit_uniform_location : require\n"
	"in vec2 texture_coordinates;"
	...
	"{"
	"	frag_color = vec4(inColor.r, inColor.g, inColor.b, inColor.a);"
	"	vec2 textureCoordTransformed = texture_coordinates * resolution;"
	"	float dist = distance(texture_coordinates, vec2(0.5, 0.5));"
	"	if(dist > 0.5)"
	"	{"
	"		frag_color.a = 0;"
	"	}"
...
const char* dfPrimitiveLineVert = ""
	"#version 130\n"
	"#extension GL_ARB_explicit_attrib_location : require\n"
	"#extension GL_ARB_explicit_uniform_location : require\n"
	"layout(location = 0) in vec3 position;"
...
const char* dfPrimitiveLineFrag = ""
	"#version 130\n"
	"#extension GL_ARB_explicit_attrib_location : require\n"
	"#extension GL_ARB_explicit_uniform_location : require\n"
	"uniform vec4 inColor;"
